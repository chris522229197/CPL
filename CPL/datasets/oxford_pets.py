...
class OxfordPets(DatasetBase):
    ...
    def subsample_classes(*args, subsample="all"):
        ...
        Args:
            args: a list of datasets and number of training shot in IR, i.e., train, val, test and trian_shot
            subsample (str): what classes to subsample.
        """
        assert subsample in ["all", "base", "new", "fewshot", "unseen", "split_two"]
        ...
        elif subsample == "fewshot":
            selected = labels[:args[3]]  # for few-shot learning
        elif subsample == "unseen":
            selected = labels[args[3]:]  # for testing
        relabeler = {y: y_new for y_new, y in enumerate(selected)}
        

        output = []

        for dataset in args[0:3]:
            dataset_new = []
            for item in dataset:
